...
class NodeData(peewee.Model):
    node_id = snapshot.ForeignKeyField(index=True, model='node', null=True)
    name = TextField()
...
def forward(old_orm, new_orm):
    nodepayments = new_orm['nodepayments']
    return [
        # Apply default value datetime.datetime(2023, 9, 5, 23, 39, 6, 405254) to the field nodepayments.payment_date,
        nodepayments.update({nodepayments.payment_date: datetime.datetime(2023, 9, 5, 23, 39, 6, 405254)}).where(nodepayments.payment_date.is_null(True)),
    ]
