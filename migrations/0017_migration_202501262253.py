...
class Interaction(peewee.Model):
    ...
    interaction_data = TextField(null=True)
    class Meta:
...
@snapshot.append
class PaymentChain(peewee.Model):
    chain_name = TextField()
    wallet_address = TextField()
    rpc_address = TextField()
    class Meta:
        table_name = "payment_chains"


@snapshot.append
class PaymentToken(peewee.Model):
    token_name = TextField(column_name='chain_name')
    chain_id = snapshot.ForeignKeyField(backref='payment_tokens', index=True, model='paymentchain')
    is_native_token = BooleanField()
    contract_address = TextField()
    abi = TextField()
    token_price = FloatField()
    obsolete = BooleanField()
    class Meta:
...
@snapshot.append
class TokenPrice(peewee.Model):
    token_id = TextField()
    payment_token_id = snapshot.ForeignKeyField(backref='tokens_prices', index=True, model='paymenttoken')
    class Meta:
