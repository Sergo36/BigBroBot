...
def forward(old_orm, new_orm):
    node = new_orm['node']
    return [
        # Apply default value datetime.date(2023, 6, 8) to the field node.expiry_date,
        node.update({node.expiry_date: datetime.date(1970, 1, 1)}).where(node.expiry_date.is_null(True)),
    ]
