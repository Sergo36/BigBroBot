...
def forward(old_orm, new_orm):
    nodetype = new_orm['nodetype']
    return [
        # Apply default value False to the field nodetype.obsolete,
        nodetype.update({nodetype.obsolete: False}).where(nodetype.obsolete.is_null(True)),
        # Apply default value 0 to the field nodetype.limit,
        nodetype.update({nodetype.limit: 0}).where(nodetype.limit.is_null(True)),
    ]
...
def backward(old_orm, new_orm):
    nodetype = new_orm['nodetype']
    return [
        # Check the field `nodetype.description` does not contain null values,
    ]
